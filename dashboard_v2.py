...
def generate_email_prompt(customer, email_type, reason, warehouse, blog):
    ...
    name = customer['氏名']
    rank = customer['会員ランク']
    genre = customer.get('得意ジャンル', '本')
    tendency = customer.get('購入傾向', '買取・購入両方')
    tier = customer.get('buyback_quality_tier', 'B')
    total_count = customer.get('累計買取回数', 0)
    total_amount = customer.get('累計買取金額', 0)
    
    campaigns = blog.get('current_campaigns', [])
    campaigns_text = "\n".join([f"・{c['name']}: {c['description']}" for c in campaigns]) if campaigns else "特になし"
    
    stories = [p for p in blog.get('blog_posts', []) if p.get('use_in_email')]
    stories_text = "\n".join([f"・{s['title']}: {s['summary']}" for s in stories[:3]]) if stories else "特になし"
    
    gifts = blog.get('gift_options', [])
    gifts_text = "\n".join([f"・{g['name']}: {g['description']}" for g in gifts[:2]]) if gifts else ""
...
- お願いの押し売り感を完全に排除
- まず感謝と価値提供から始める
- 選択肢を提示し、決定権はお客様にあることを明示
- 「もしよろしければ」「ご検討いただけると幸いです」などの柔らかい表現を使用"""
        relationship_context = f"過去に少しご連絡が多くなってしまったかもしれません。今回は特別なお知らせとしてお届けしています。"
    elif balance > 20:
        tone_instruction = """
【トーン：親密で感謝に満ちた】
- 長年のお付き合いへの深い感謝を表現
...
- パーソナルな語りかけ
- VIP感・特別感を演出"""
        relationship_context = f"いつもバリューブックスをご利用いただき、心より感謝申し上げます。{rank}会員の{name}様だからこそ、いち早くお届けしたい情報がございます。"
    else:
        tone_instruction = """
【トーン：温かみと人間味】
- 友人に手紙を書くような自然さ
- 本への愛情が伝わる言葉選び
- 企業的な硬さを避け、人の温もりを感じさせる"""
        relationship_context = f"{name}様、いかがお過ごしでしょうか。"
    
    # メールタイプ別の詳細指示
    type_instructions = {
        "💸 買取促進": f"""
【目的】買取のご依頼をいただく
【重要】押し売りではなく「本の新しい旅立ち」「次の読者への橋渡し」という価値を伝える

【構成】
1. 季節の挨拶 or 時候の言葉（1-2文）
2. 関係性への言及「{relationship_context}」
3. 本棚の整理や断捨離の気持ちよさに共感する文章
4. キャンペーン情報（あれば自然に組み込む）
5. {genre}に関連した具体的なエピソードや価値
6. 買取の簡便さ・メリットの提示
7. 行動喚起（柔らかく、選択肢として提示）
8. 感謝の結び""",
        
        "🎁 プレゼント": f"""
【目的】関係性の構築・感謝の表現（見返りを求めない）
【重要】完全に「お客様へのギフト」として。買取依頼は一切含めない

【構成】
1. 心温まる挨拶
2. 「日頃のご愛顧への感謝として」という前置き
3. プレゼント/特典の詳細（{gifts_text}）
4. なぜあなたにお届けしたかったか（パーソナルな理由）
5. 本にまつわる素敵なエピソードやストーリー
6. 心からの感謝の言葉""",
        
        "📰 ニュース": f"""
【目的】価値ある情報提供・関係性維持
【重要】売り込み感ゼロ。純粋に面白い/役立つ情報として

【構成】
1. 親しみやすい挨拶
2. 「{name}様にぜひ知っていただきたいお話が」という導入
3. ニュース/ストーリーの詳細（{stories_text}）
4. なぜこの情報が{genre}好きの{name}様に関係するか
5. 読後感を良くする温かい締め""",
        
        "⚠️ 緊急買取": f"""
【目的】できるだけ多くの買取依頼を獲得
【重要】緊急でも押し付けがましくならない。誠実さを保つ

【構成】
1. 率直な挨拶
2. 現状の説明（在庫が少なく、多くの読者が待っている）
3. 「{name}様のお力をお借りできれば」という謙虚なお願い
4. 買取価格アップなどの特典
5. {genre}の本が特に求められていることへの言及
6. 手続きの簡便さ
7. 感謝と敬意を込めた結び"""
    }
    
    type_guide = type_instructions.get(email_type, type_instructions["📰 ニュース"])
    
    prompt = f"""# バリューブックス メールライティング指示書

あなたは「バリューブックス」の最高のメールライターです。
単なるマーケティングメールではなく、**一人の人間から一人の人間への手紙**を書いてください。

## 🎯 ミッション
「本を通じて人と人をつなぐ」というバリューブックスの理念を体現し、
受け取った方が「このメール、なんだか嬉しいな」と感じる文章を書くこと。
...
得意ジャンル: {genre}
利用傾向: {tendency}
品質ティア: {tier}（A=最優良, B=良好, C=注意, D=要配慮）
累計お取引: {total_count}回 / ¥{total_amount:,}
関係性スコア: {balance:+d}（{'とても良好' if balance > 20 else '良好' if balance > 0 else '要ケア'}）
```
...
### DO（やるべきこと）
- 具体的なエピソードや情景を描写する
- 「〜ではないでしょうか」「〜かもしれません」など読者に考えさせる問いかけ
- 本への愛情が伝わる表現（「あの一冊との出会い」「ページをめくる喜び」など）
- 季節感や時事を自然に織り込む
- {genre}に関する具体的な言及
- 手書きの手紙のような温かみ
- 読んでいて心地よいリズム
...
> 山田様、桜の便りが届く季節となりました。
> 
> 先日、倉庫で一冊のビジネス書を手に取った時のことです。
> 少し日焼けしたその本には、たくさんの付箋と書き込みがありました。
> きっと、前の持ち主の方が何度も読み返されたのでしょう。
> 
> 私たちは、こうした「誰かの大切だった一冊」を、
> 次の読者へとつなぐ仕事をしています。
> 
> もし山田様のお手元にも、役目を終えた本がございましたら、
> 新しい読者のもとへ届けるお手伝いをさせてください。

## 📝 出力フォーマット

件名: 【25文字以内・興味を引くが煽らない】
...
上記すべてを踏まえ、{name}様の心に響く最高のメールを作成してください。"""
...
def generate_with_gpt(prompt, api_key=None):
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": """あなたは「バリューブックス」の最高のメールライターです。

あなたの使命は、受け取った人が思わず微笑んでしまうような、心温まるメールを書くことです。
